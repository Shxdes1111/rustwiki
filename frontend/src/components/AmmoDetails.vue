...
import { useWeaponStore, type AmmoDetail } from '../stores/weapons'
...
const ammo = ref<AmmoDetail | null>(null)
  ...
  <div v-if="ammo" class="wiki-page">
    ...
    <div class="wiki-layout">
      ...
      <aside class="infobox">
        ...
        <div class="infobox-row"><span>ID</span><span class="value">{{ ammo.id }}</span></div>
        <div class="infobox-row"><span>Name</span><span class="value">{{ ammo.name }}</span></div>

        <div class="infobox-section-title">Compatible Weapons</div>
        <div class="infobox-grid">
          <router-link v-for="weapon in ammo.compatible_weapons" :key="weapon.id" :to="`/weapon/${weapon.id}`" class="grid-item" :title="weapon.name">
            🔫 <span class="grid-item-text">{{ weapon.name }}</span>
          </router-link>
          <div v-if="!ammo.compatible_weapons?.length" class="empty-text">No compatible weapons</div>
        </div>
      </aside>
    </div>
  </div>
